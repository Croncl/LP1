...
Outsider::Outsider(const sf::Texture& texture) 
    : speed(80.0f), health(10), shootInterval(sf::seconds(1.9f)), markedForRemoval(false) {
    ...
    // Construir o nome do arquivo da textura do projétil
    std::ostringstream oss;
    oss << "assets/projectilO00" << randomIndex << ".png";
    std::string projectileTextureFile = oss.str();

    // Carregar a textura do projétil
    if (!projectileTexture.loadFromFile(projectileTextureFile)) {
        throw std::runtime_error("Failed to load projectile texture: " + projectileTextureFile);
    }
}
